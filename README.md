...
>
> Rooting can brick or boot-loop the device, void or complicate warranty/service, weaken platform security, break DRM, Play Integrity/attestation, banking or streaming apps, and interfere with BOOX OTA updates. Privacy/debloat profiles intentionally disable some BOOX online features. Firmware updates can invalidate every device-specific assumption in this repository.
>
...
| Vendor Lockdown | Systemlessly removes cloud sync and denies WAN for every recorded/discovered `com.onyx` UID, including shared UID 1000 |
...
| **Vendor Lockdown (maximum privacy)** | Applies Purge, systemlessly removes `ksync`, and denies WAN for every recorded and dynamically discovered `com.onyx` UID—including shared UID 1000—over IPv4 and IPv6. Reasserts packages/settings every 60 seconds. | BOOX Cloud/sync, online OTA checks, and internet features inside BOOX-owned apps will not work. Restore before OTA. |
...
Balanced and Purge refuse unsafe per-app firewall rules for UIDs below 10000 because the launcher and `ksync` share Android UID 1000. Vendor Lockdown is the explicit exception: maximum privacy intentionally blocks UID 1000 from WAN while allowing loopback, RFC1918, link-local, multicast/broadcast, and IPv6 ULA destinations. The dedicated Magisk `/system/etc/hosts` overlay and hardcoded bootstrap-IP rejection apply independently. EasyTransfer retains only local-network access in Balanced, Purge, and Vendor Lockdown.
...
Vendor Lockdown is the strongest internet-capable mode. At installation it records all current BOOX UIDs into the module, installs their WAN-deny rules before Android finishes booting, rediscovers `com.onyx` packages after startup, and checks again every 60 seconds. UID 1000 is intentionally included, so BOOX Cloud/sync, online OTA checks, and internet features inside BOOX-owned apps will not work. Restore before an OTA. This is a direct-network guarantee for BOOX package UIDs, not a claim that arbitrary third-party applications can never contact Chinese infrastructure. A literal device-wide guarantee requires disabling all WAN traffic.
